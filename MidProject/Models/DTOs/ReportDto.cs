...
public class ReportDto
{
    ...
    public string DefaultNotificationContent => GetDefaultNotificationContent(Status);

    public string GetDefaultNotificationContent(string outcome)
    {
        var approved = outcome == "Approved";
        var outcomeDisplay = approved ? "檢舉成立" : "駁回檢舉";
        var resultLine = approved
...
{
    [Required]
    [RegularExpression("Approved|Rejected", ErrorMessage = "狀態值不正確")]
    public string Status { get; set; } = string.Empty;
    ...
    [Required(ErrorMessage = "請輸入通知檢舉者的標題")]
    [StringLength(100)]
    public string ReporterNotificationTitle { get; set; } = string.Empty;

    [Required(ErrorMessage = "請輸入通知檢舉者的內容")]
    [StringLength(1000)]
    public string ReporterNotificationContent { get; set; } = string.Empty;

    [StringLength(100)]
    public string? ReportedMemberNotificationTitle { get; set; }

    [StringLength(1000)]
    public string? ReportedMemberNotificationContent { get; set; }
}
